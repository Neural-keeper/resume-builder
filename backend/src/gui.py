...
# --- State Helpers ---
def refresh_all():
    """Triggers an app-wide interface refresh."""
    ui.notify("Changes saved!", type="positive", position="bottom-right")
...
def main_page():
    ...
    with ui.header().classes('items-center justify-between bg-primary text-white p-4'):
        ui.label('Master Resume Builder').classes('text-xl font-bold')
        ui.button('Save All', icon='save', on_click=lambda: (manager.save(), refresh_all())).props('flat color=white')

    with ui.left_drawer(value=True).classes('bg-slate-50 w-64 p-4'):
        ui.label('Navigation').classes('text-xs font-bold text-slate-400 uppercase mb-2')
    ...
    with ui.tab_panels(tabs, value=tab_profile).classes('w-full p-6'):
        ...
        with ui.tab_panel(tab_profile):
            ...
            ui.button('Save Profile', on_click=save_profile, icon='save').classes('mt-4')
            ...
            def render_summaries():
                for key, summary_text in manager.data['summaries'].items():
        ...
        with ui.tab_panel(tab_exp):
            ...
            def render_experiences():
                for exp in manager.data['experiences']:
                    with ui.card().classes('w-full mb-4 p-4'):
                        with ui.row().classes('justify-between w-full'):
                            ui.label(f"{exp['role']} @ {exp['company']}").classes('font-bold text-lg')
                        ...
                        ui.label(f"{exp['dates']} | {exp['location']}").classes('text-sm text-slate-500 mb-2')
            ...
            # Add New Experience Dialog / Form
        ...
        with ui.tab_panel(tab_edu):
            ...
            def render_education():
                for ed in manager.data['education']:
                    with ui.card().classes('w-full mb-4 p-4'):
                        with ui.row().classes('justify-between w-full'):
                            ui.label(f"{ed['degree']} in {ed['major']}").classes('font-bold text-lg')
                        ...
                        ui.label(f"{ed['uni']} ({ed['start']} - {ed['end']})").classes('text-slate-600')
                        if ed.get('minor'):
                            ui.label(f"Minor: {ed['minor']}").classes('text-sm text-slate-500')
        ...
        with ui.tab_panel(tab_proj):
            ...
            @ui.refreshable
            def render_projects():
                for proj in manager.data['projects']:
                    with ui.card().classes('w-full mb-4 p-4'):
                        with ui.row().classes('justify-between w-full'):
                            ui.label(proj['title']).classes('font-bold text-lg')
                        ...
                        ui.label(proj['description']).classes('text-slate-700 mb-2')
                        if proj.get('link'):
        ...
        with ui.tab_panel(tab_skills):
            ...
            @ui.refreshable
            def render_skills():
                for category, skill_list in manager.data['skills'].items():
                    with ui.card().classes('w-full mb-4 p-4'):
        ...
        with ui.tab_panel(tab_certs):
            ...
            @ui.refreshable
            def render_certs():
                for cert in manager.data['certifications']:
                    with ui.card().classes('w-full mb-4 p-4'):
                        with ui.row().classes('justify-between w-full'):
                            ui.label(cert['name']).classes('font-bold text-lg')
                        ...
                        ui.label(f"Issuer: {cert['issuer']} | Issued: {cert['date']}").classes('text-slate-600')
...
# Run the app locally
ui.run(title='Master Resume Manager', port=8080, reload=True)